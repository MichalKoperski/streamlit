...
# ---------------------------------------------------------
# 3. Mini-notebook (bezpieczny) – STABILNA wersja dla Cloud
# ---------------------------------------------------------
elif page == "🧪 Notebook (bezpieczny)":

    try:
        st.title("🧪 Mini-Notebook (bezpieczny)")
        st.caption("Wiele komórek, Run/Run all, zapis/odczyt JSON. Bez importów/systemu.")
        ...
        with colR1:
            if st.button("🔄 Reset notebooka"):
                for k in ["nb_cells", "nb_next_id", "nb_ns", "nb_df"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()
        with colR2:
            st.info("Jeśli coś się wysypie po zmianach kodu, kliknij Reset notebooka.")
        ...
        if data_source == "📤 Upload CSV":
            ...
            wwe_path = "wwe.csv"
            wcw_path = "wcw.csv"
            available = [p for p in [wwe_path, wcw_path] if os.path.exists(p)]
            if not available:
        ...
        # -------------------------
        # Notebook state (lightweight)
        # -------------------------
        if "nb_cells" not in st.session_state:
            st.session_state["nb_cells"] = [
                {"id": 1, "title": "Komórka 1", "code": "out_df = df.head(10)\n# fig = px.histogram(df, x=df.columns[0])", "status": ""}
            ]
            st.session_state["nb_next_id"] = 2

        if "nb_ns" not in st.session_state:
            st.session_state["nb_ns"] = {}  # namespace shared across cells (like notebook)

        def add_cell():
            cid = st.session_state["nb_next_id"]
            st.session_state["nb_next_id"] += 1
            st.session_state["nb_cells"].append(
                {"id": cid, "title": f"Komórka {cid}", "code": "# out_df = ...\n# fig = ...", "status": ""}
            )
        ...
        with s2:
            nb_export = {"version": 1, "cells": [{"id": c["id"], "title": c["title"], "code": c["code"]} for c in st.session_state["nb_cells"]]}
            st.download_button(
        ...
        with s3:
            ...
            if nb_file is not None:
                try:
                    ...
                    new_cells = []
                    max_id = 0
                    for c in loaded.get("cells", []):
                        cid = int(c.get("id", 0))
                        max_id = max(max_id, cid)
                        new_cells.append({"id": cid, "title": str(c.get("title", f"Komórka {cid}")), "code": str(c.get("code", "")), "status": ""})
                    if not new_cells:
        ...
        # Execution
        # -------------------------
        def run_code(code: str) -> tuple[object, object, str]:
            ok, reason = code_is_safe(code)
            if not ok:
                return None, None, f"❌ Zablokowano: {reason}"

            ns = st.session_state.get("nb_ns", {})
            global_env = {"__builtins__": SAFE_BUILTINS, "df": df, "pd": pd, "px": px}
            ...
            try:
                ...
                # merge locals into globals, persist
                for k, v in local_env.items():
                    global_env[k] = v
                st.session_state["nb_ns"] = {k: v for k, v in global_env.items() if k not in ["__builtins__"]}

                out_df = st.session_state["nb_ns"].get("out_df")
                fig = st.session_state["nb_ns"].get("fig")
                return out_df, fig, "✅ Wykonano"
            except Exception as e:
                return None, None, f"❌ Błąd: {e}"

        t1, t2 = st.columns([1, 1])
        with t1:
            run_all = st.button("▶️ Run all", type="primary")
        with t2:
            if st.button("🧹 Wyczyść zmienne"):
                st.session_state["nb_ns"] = {}
                st.success("Wyczyszczono namespace.")

        if run_all:
            for i in range(len(st.session_state["nb_cells"])):
                out_df, fig, status = run_code(st.session_state["nb_cells"][i]["code"])
                st.session_state["nb_cells"][i]["status"] = status
        ...
        for i, cell in enumerate(st.session_state["nb_cells"]):
            ...
            h1, h2, h3, h4 = st.columns([1.2, 0.3, 0.3, 0.3])
            with h1:
                cell["title"] = st.text_input("Tytuł", value=cell["title"], key=f"nb_title_{cell['id']}")
            with h2:
            ...
            cell["code"] = st.text_area("Kod", value=cell["code"], height=170, key=f"nb_code_{cell['id']}")

            cRun, cStatus = st.columns([1, 3])
            with cRun:
                if st.button("▶️ Run", key=f"nb_run_{cell['id']}"):
                    out_df, fig, status = run_code(cell["code"])
                    cell["status"] = status
            with cStatus:
                if cell.get("status"):
                    st.write(cell["status"])

            # Show outputs (shared namespace variables)
            ns = st.session_state.get("nb_ns", {})
            if "out_df" in ns and ns["out_df"] is not None:
                st.markdown("**out_df**")
                if isinstance(ns["out_df"], pd.DataFrame):
                    st.dataframe(ns["out_df"], use_container_width=True, height=320)
                else:
                    st.write(ns["out_df"])

            if "fig" in ns and ns["fig"] is not None:
                st.markdown("**fig**")
                st.plotly_chart(ns["fig"], use_container_width=True)
        ...
        st.error("Notebook page crashed.")
        st.exception(e)
        st.info("Kliknij ‘Reset notebooka’ albo sprawdź logi w Streamlit Cloud: Manage app → Logs.")
